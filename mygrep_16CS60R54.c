	 ...
	 //  return false;
	   ...
	   return matchExp(exp, str, i + 1, j + 1);
	else if (exp[i] == '*')
        {
          ...
          // if(i < strlen(exp))
          //   if(exp[i] == str[j-1])
          //        return matchExp(exp, str, i, j-1);
           return matchExp(exp , str, i , j);
        }      
        else if(exp[i] == '+')
        {
               bool flag = false;
               ...
               i++;
            //   if(count > 1 && i < strlen(exp))
             //   if(exp[i] == str[j-1])
              //    return matchExp(exp, str, i, j-1);
               if(flag)
                  return matchExp(exp , str , i , j);
               else
                  return matchExp(exp, str , 0, j);
        }
        else if(exp[i] == '?')
        {
              int count = 0;
...
//					|| matchExp(exp, str, i + 1, j + 1);
        ...
        else if(exp[i] != str[j] && exp[i+1] == '+')
           return false;
        else if((exp[i] != str[j]) && exp[i+1]!='*' &&  exp[i+1] != '+' && exp[i+1] !='?') 
        {  
            if(i != 0)
               return matchExp(exp, str, 0 , j);
           else 
              return matchExp(exp , str , 0 , j+1);
        }
        else 
             return  matchExp(exp , str , i+2 , j);
