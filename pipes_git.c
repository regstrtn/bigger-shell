...
int main()
{
	...
	while (1) {
		...
		while (next != NULL) {
			...
			*next = '\0';
			printf("CMD: %s Next: %s\n", cmd, next+1);
			input = run(cmd, input, first, 0);         //Input has been modified here
 
			cmd = next + 1;
			next = strchr(cmd, '|'); /* Find next '|' */
			first = 0;																//first call already executed, now set first to zero
		}
		input = run(cmd, input, first, 1); //This is the last call
		cleanup(n);
		n = 0;
	}
	return 0;
